...
import { TENANT_SCHEMA } from '../../sales.tokens.js';
...
export class PrismaSaleRepository implements SaleRepositoryPort {
  ...
  async checkout(input: CheckoutInput): Promise<Sale> {
    return this.tenantPrisma.withTenant(async (tx) => {
      ...
      await tx.$executeRawUnsafe(
        ...
        input.branchCode,
        input.userId,
        input.cashierSessionId ?? null,
        input.totals.numberSeq,
  ...
  private mapToSale(input: CheckoutInput): Sale {
    ...
    const payments = input.payments.map(
      (p: { method: PaymentMethod; amount: number; ref?: string }) =>
        SalePayment.rehydrate({
          method: p.method,
          amount: p.amount,
          ref: p.ref,
        }),
  ...
  // biome-ignore lint/suspicious/noExplicitAny: raw SQL queries on tx client
  private async fetchPayments(tx: any, saleId: string): Promise<Record<string, unknown>[]> {
    const result = await tx.$queryRawUnsafe(
  ...
  async registerReturn(input: ReturnInput): Promise<SaleReturn> {
    return this.tenantPrisma.withTenant(async (tx) => {
      await tx.$executeRawUnsafe(
        ...
        input.saleId,
        input.reason ?? null,
        JSON.stringify(input.items.map((i: SaleItem) => i.toDTO())),
        input.total,
      );
      ...
      for (const item of input.items) {
        await tx.$executeRawUnsafe(
          ...
          (await this.getSaleBranch(tx, input.saleId)) ?? '',
          item.productId,
          item.qty,
        );
  ...
  private hydrateReturn(row: any): SaleReturn {
    // biome-ignore lint/suspicious/noExplicitAny: parsed JSON items

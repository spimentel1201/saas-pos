...
export function Receipt({ config, data }: ReceiptProps) {
  ...
  const _sep = buildSeparator(W);
  const _thinSep = buildSeparator(W, '.');

  return (
    <div className={`receipt receipt--${config.width === '80mm' ? '80mm' : '58mm'}`}>
      {/* Header */}
      <div className="receipt-header">
        {config.logoUrl && <img src={config.logoUrl} alt="Logo" className="receipt-logo" />}
        {config.businessName && <div className="receipt-business">{config.businessName}</div>}
      ...
      <div className="receipt-info">
        ...
        {'\n'}
        {'Fecha: '}
        {formatSaleDate(data.createdAt)} {formatSaleTime(data.createdAt)}
        {'\n'}
        {'Cajero: '}
        {data.cashierName}
        {'\n'}
          ...
          return (
            <div key={idx} className="receipt-item">
              <div className="receipt-item-row">
                ...
                <span className="receipt-item-name">{padRight(nameLines[0] ?? '', W - 14)}</span>
                <span className="receipt-item-qty-price">
